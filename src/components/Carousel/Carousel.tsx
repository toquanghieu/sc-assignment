...
const Carousel: React.FC<CarouselProps> = ({
  ...
  const changeSlide = useCallback(
    (direction: 'prev' | 'next') => {
      ...
      if (
        visibilityState &&
        (hidden || !isVisible(carouselRef.current as unknown as HTMLElement))
      ) {
    ...
    [getNextElement, slide, visibilityState],
  );
  ...
  useEffect(() => {
    ...
    if (
      (stopSliding ?? (ride && isFirstSlide.current)) ||
      !ride ||
